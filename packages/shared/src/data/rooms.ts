...
// Total: 65 unique rooms (45 2nd Edition + 20 Widow's Walk)
...
export interface Room {
  ...
  name: string;
  nameEn: string;
  floor: Floor;  // 主要樓層（向後兼容）
  floors?: Floor[];  // 房間可以出現在多個樓層（可選）
  symbol: SymbolType;
  ...
  // 擴充屬性
  set?: '2nd' | 'ww';
  roomType?: 'omen' | 'event' | 'item' | 'none' | 'landing' | 'special';
  specialText?: string;
  // 向後兼容
...
// 地板
const floorRect = (c: string) => `<rect x="8" y="8" width="84" height="84" fill="${c}" stroke="#2A2A2A" stroke-width="2"/>`;
...
// 輔助函數：生成圖標
const makeIcon = (color: string, doors: string[], symbol?: string | null) => {
  const doorSvgs = doors.map(d => {
    if (d === 'north') return doorN;
    if (d === 'south') return doorS;
    if (d === 'east') return doorE;
    if (d === 'west') return doorW;
    return '';
  }).join('');
  const symSvg = symbol === 'E' ? symE : symbol === 'I' ? symI : symbol === 'O' ? symO : '';
  return `${floorRect(color)}${doorSvgs}${symSvg}`;
};

// 輔助函數：取得第一個樓層作為主要樓層
const getPrimaryFloor = (floors: Floor[]): Floor => floors[0] || 'ground';

// ==================== ALL 65 OFFICIAL ROOMS ====================
// 2nd Edition (45 rooms) + Widow's Walk (20 rooms)
...
// 轉換為 Room 介面格式
function convertToRoom(data: any): Room {
  const primaryFloor = getPrimaryFloor(data.floors);
  return {
    ...data,
    floor: primaryFloor,
    roomType: data.type,
    icon: makeIcon(data.color, data.doors, data.symbol),
    gallerySvg: `/gallery/rooms/${data.id}.svg`,
    type: undefined, // 清除衝突的 type 屬性
  };
}

// ==================== EXPORT ALL ROOMS ====================
export const ALL_ROOMS: Room[] = ALL_ROOMS_DATA.map(convertToRoom);
...
// ==================== ROOMS BY FLOOR ====================
export const GROUND_ROOMS = ALL_ROOMS.filter(r => r.floors?.includes('ground') || r.floor === 'ground');
export const UPPER_ROOMS = ALL_ROOMS.filter(r => r.floors?.includes('upper') || r.floor === 'upper');
export const BASEMENT_ROOMS = ALL_ROOMS.filter(r => r.floors?.includes('basement') || r.floor === 'basement');
export const ROOF_ROOMS = ALL_ROOMS.filter(r => r.floors?.includes('roof') || r.floor === 'roof');
...
export const ROOM_STATS = {
  ...
  byType: {
    omen: ALL_ROOMS.filter(r => r.roomType === 'omen').length,
    event: ALL_ROOMS.filter(r => r.roomType === 'event').length,
    item: ALL_ROOMS.filter(r => r.roomType === 'item').length,
    none: ALL_ROOMS.filter(r => r.roomType === 'none').length,
    landing: ALL_ROOMS.filter(r => r.roomType === 'landing').length,
    special: ALL_ROOMS.filter(r => r.roomType === 'special').length,
  },
};

// 向後兼容
export const ROOMS = ALL_ROOMS;
export const OFFICIAL_ROOMS = ALL_ROOMS;

// 多樓層房間（向後兼容）
export const MULTI_FLOOR_ROOMS: Room[] = ALL_ROOMS.filter(r => (r.floors?.length || 0) > 1);
...
export class RoomDeck {
  ...
  reset() {
    this.ground = [...GROUND_ROOMS.filter(r => r.roomType !== 'landing')];
    this.upper = [...UPPER_ROOMS.filter(r => r.roomType !== 'landing')];
    this.basement = [...BASEMENT_ROOMS.filter(r => r.roomType !== 'landing')];
    this.drawn.clear();
    this.shuffleAll();
  }
...
export function getRandomRoom(): Room {
  const available = ALL_ROOMS.filter(r => r.roomType !== 'landing');
